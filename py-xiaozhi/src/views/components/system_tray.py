...
class SystemTray(QObject):
    ...
    def _setup_tray(self):
        ...
        try:
            ...
            # 显示系统托盘图标
            self.tray_icon.show()
            self.logger.info("Biểu tượng khay hệ thống đã được khởi tạo")

        except Exception as e:
            self.logger.error(f"Khởi tạo biểu tượng khay hệ thống thất bại: {e}", exc_info=True)
    ...
    def _create_tray_menu(self):
        ...
        # 添加设置菜单项
        settings_action = QAction("Cấu hình", self.parent_widget)
    ...
    def update_status(self, status: str, connected: bool = True):
        ...
        try:
            ...
            # 设置提示文本
            tooltip = f"Trợ lý AI Xiaozhi - {status}"
            self.tray_icon.setToolTip(tooltip)

        except Exception as e:
            self.logger.error(f"Cập nhật biểu tượng khay hệ thống thất bại: {e}")
    ...
    def _get_status_color(self, status: str, connected: bool) -> QColor:
        ...
        if "lỗi" in status.lower():
            return QColor(255, 0, 0)  # Đỏ - trạng thái lỗi
        elif "đang lắng nghe" in status:
            return QColor(255, 200, 0)  # Vàng - trạng thái đang lắng nghe
        elif "đang nói" in status:
            return QColor(0, 120, 255)  # Xanh dương - trạng thái đang nói
        else:
            return QColor(0, 180, 0)  # Xanh lá - trạng thái sẵn sàng/đã khởi động
