    ...
    def capture(self) -> bool:
        ...
        try:
            ...
            for i in range(warmup_frames):
                ...
                if ret and np.mean(temp_frame) > 0:
                    logger.info(f"Camera warmup frame {i+1}: successful")
                    break
                else:
                    logger.info(f"Camera warmup frame {i+1}: still black, continuing...")
                    time.sleep(0.1)
            ...
            # 检查图像是否仍然为黑屏
            if np.mean(frame) < 1:
                logger.error("Captured image is still black after warmup")
                return False
